...
function notificationTypeFromSeverity(severity?: string): MapNotification['type'] {
  const value = String(severity ?? '').toLowerCase();

  if (value === 'critical' || value === 'kritis' || value === 'high') return 'critical';
  if (value === 'warning' || value === 'sedang' || value === 'medium') return 'warning';
  if (value === 'resolved' || value === 'rendah' || value === 'low') return 'success';
...
function getSeverityRiskLevel(severity?: string): 'HIGH' | 'MEDIUM' | 'LOW' {
  ...
  if (value === 'KRITIS' || value === 'CRITICAL' || value === 'HIGH') return 'HIGH';
  if (value === 'SEDANG' || value === 'WARNING' || value === 'MEDIUM') return 'MEDIUM';
...
export function MapPage() {
  ...
  const [searchPin, setSearchPin] = useState<{ coords: [number, number]; label: string } | null>(
    null,
  );
  ...
  const criticalNotification = notifList.find((notification) => notification.type === 'critical');
    ...
    <div className="relative w-screen h-screen overflow-hidden bg-[var(--bg-main)] text-[var(--text-main)]">
      ...
      <div
        className={`map-topbar absolute ${
          criticalNotification ? 'top-[36px]' : 'top-0'
      ...
      <div
        className={`absolute ${
          criticalNotification ? 'top-[96px]' : 'top-[60px]'
      ...
      <div
        className={`absolute ${
          criticalNotification ? 'top-[148px]' : 'top-[112px]'
      ...
      <div
        className={`absolute ${
          criticalNotification ? 'top-[200px]' : 'top-[164px]'
          ...
          <div className="px-4 py-3">
            ...
            {clickRiskError && (
              <div className="py-2">
                ...
                <p className="text-[var(--danger)] text-xs mt-1">{clickRiskError}</p>
              </div>
            )}
            ...
            {clickRiskData && (
              <div className="space-y-2">
                <p className="text-[var(--text-muted)] text-[10px]">
                  {clickRiskData.location.kelurahan}, {clickRiskData.location.kecamatan}
                </p>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    ...
                    <p
                      ...
                      style={{ color: riskLevelColor(clickRiskData.risk.risk_level) }}
                    >
                ...
                <div className="grid grid-cols-2 gap-2 mt-1">
                  <div className="bg-[var(--bg-soft)] rounded-lg px-2.5 py-1.5">
                    ...
                    <p className="text-xs font-bold text-[var(--text-main)]">
                      {(clickRiskData.components.weather_score * 100).toFixed(0)}%
                    </p>
                  </div>
                  ...
                  <div className="bg-[var(--bg-soft)] rounded-lg px-2.5 py-1.5">
                    ...
                    <p className="text-xs font-bold text-[var(--text-main)]">
                      {clickRiskData.details.water_station.water_level?.toFixed(0) ?? 'N/A'} cm
                    </p>
                  </div>
                </div>
              </div>
...
function MapLayerPanel({
  ...
  return (
    <div className="absolute top-full mt-2 left-0 w-52 bg-[var(--bg-card)] border border-[var(--border-soft)] rounded-xl overflow-hidden backdrop-blur-sm z-10">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--border-soft)]">
        <span className="text-[var(--text-main)] text-sm font-semibold">Map Layers</span>
      ...
      <div className="px-4 py-3 space-y-3">
        <MapLayerToggle label="Risk Zones" value={showRiskZones} setter={setShowRiskZones} />
        <MapLayerToggle
          label="Active Reports"
          value={showActiveReports}
          setter={setShowActiveReports}
        ...
        <div className={dropdownClass}>
          {suggestions.length === 0 && query.length >= 2 && !loading ? (
            ...
            suggestions.map((suggestion) => (
              ...
              >
                <MapPin size={12} className="text-[var(--accent)] shrink-0 mt-0.5" />
